...
object SoftImport {
    ...
    private fun boxOf(layout: PlotLayout): Box? {
    ...
    private fun shifted(layout: PlotLayout, offsetX: Int, offsetY: Int): PlotLayout {
        if (offsetX == 0 && offsetY == 0) return layout

        val moved = PlotLayout(id = layout.id, name = layout.name, size = layout.size)
    ...
    private fun laidOver(existing: PlotLayout, incoming: PlotLayout, turns: Int, offsetX: Int, offsetY: Int, box: Box?): Fit {
     ...
     */
    private fun snugnessOf(existing: PlotLayout, box: Box): Int {
